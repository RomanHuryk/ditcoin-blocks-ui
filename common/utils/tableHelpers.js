...
export const formatAmountFromData = (ouputs) => {
    ...
    for (let i = 0, limit = ouputs.length; i < limit; i++) {
        ...
        if (outputCopies[i].xmr_outputs) {
            outputCopies[i].dit_outputs = formatAmount(outputCopies[i].xmr_outputs);
            delete outputCopies[i].xmr_outputs;
        }
        if (outputCopies[i].xmr_inputs) {
            outputCopies[i].dit_inputs = formatAmount(outputCopies[i].xmr_inputs);
...
export const formatDITColumns = (columns) => {
    let columnCopies = [];
    for (let i = 0, limit = columns.length; i < limit; i++) {
        columnCopies.push(Object.assign({}, columns[i]));
        if (columnCopies[i].header === 'xmr_outputs') columnCopies[i].header = columnCopies[i].accessor = 'dit_outputs';
        if (columnCopies[i].header === 'xmr_inputs') columnCopies[i].header = columnCopies[i].accessor = 'dit_inputs';
    }
